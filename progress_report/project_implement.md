以下是在阿里云 ECS gn7i (ecs.gn7i-c8g1.2xlarge) 实例上，从零开始部署 LAKE（Machine Learning–Assisted OS）项目、进行实验验证、后续开发的规划。
...
3. **本地环境**  
   - 一台能联网的 Linux/macOS/Windows（带 WSL）终端。（本项目本地为windows）  
   - 安装 `ssh` 客户端、`git`。  
   - 有需要时，配置 VPN 或安全组规则保证能访问 GitHub 与 NVIDIA 官网。
...
1. **登录阿里云控制台** → **ECS 实例** → **创建实例**  
   ...
   - **实例规格**： `gn7i`，选择 **ecs.gn7i-c8g1.2xlarge**（1×A10，8 vCPU，30 GiB 内存）。  
   - **网络**：  
     - VPC、交换机按需。  
     - 安全组放通：  
       - SSH 22/TCP  
...
根据LAKE项目在github的代码库中的 README.md，安装必要包（编译工具链和内核模块依赖）citeturn0file0：
...
- **说明**：
  ...
  - `libncurses-dev` 等：配置内核菜单与模块编译需要的。
  - `dkms`：自动重建内核模块时有用。
...
> **需要注意的要点**：每次重编译内核后，需重新安装驱动（可重复上述后两条命令）。
...
- **内核重编译**：每次 `full_compilation.sh` 后，重装 NVIDIA 驱动；  
- **持久存储**：若实验后发现数据量大，可额外挂载高 IOPS 云盘或使用 OSS 作对象存储；  
